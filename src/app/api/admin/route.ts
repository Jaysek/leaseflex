...
export async function GET(request: NextRequest) {
  ...
  const [offersRes, waitlistRes, subscriptionsRes, claimsRes] = await Promise.all([
    ...
    supabase.from('waitlist').select('id, created_at, email, drip_step').order('created_at', { ascending: false }).limit(100),
